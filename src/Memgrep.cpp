...
void banner() {
	...
	fprintf (stdout,"Memgrep v2.1 - a tool to grep accross processes on Windows\n");
...
#endif
{
	...
	if(ReadProcessMemory(hProcess,(LPVOID)lngAddress,strBuffer,szSize,&szBytesRead)==0){
		if(GetLastError()!=299)	fprintf(stderr,"[!] Failed to read process memory %d at %0x8d read %ld\n",GetLastError(),lngAddress,szBytesRead);
	} else {
		...
		while(strBufferNow<strBufferEnd){
			...
			if (memcmp(strString,strBufferNow,strlen(strString)) == 0){
				fprintf(stdout,"[*] Got ASCII hit for %s at %0x8d in %s (%d) page starts at %0x8d ",strString,lngAddress+intCounter,strProc, dwPID,lngAddress);
				...
				if(bMatch) {
					fprintf(stdout,"[*] Got unicode hit for %s at %0x8d in %s (%d) page starts at %0x8d ",strString,lngAddress+intCounter,strProc, dwPID,lngAddress);
					PrintMemInfo(memMeminfo);
					if(bDumpHex) {
...
// Notes	: 
// 
void OpenAndGrep(bool bASCII, bool bUNICODE, char* strString, DWORD dwPID, char* strProcessNAme = nullptr)
{
	DWORD dwRet, dwMods;
	HANDLE hProcess;
	...
	bool cont = true;
	if(strProcessNAme != nullptr){
		if(strcmp(strProcessNAme,cProcess) == 0){
			cont = true;
		}else{
			cont = false;
		}
	...
	if(!cont) { fprintf(stdout,"[i] Skipping %s - %d\n",cProcess, dwPID); return ;}
...
// Notes	: 
// 
void EnumerateProcesses(bool bASCII, bool bUNICODE, char* strString, char* strProcessName = nullptr)
{
	...
	for(intCount=0;intCount<dwPIDS;intCount++)
	{
		if(dwPIDArray[intCount] != GetCurrentProcessId()){
			OpenAndGrep(bASCII,bUNICODE,strString,dwPIDArray[intCount],strProcessName);
		} else {
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	while ((chOpt = getopt(argc, argv, _T("s:p:f:a:b:n:xhq"))) != EOF) 
	switch(chOpt)
	{
		case _T('s'):
			strString = optarg;
	...
	fprintf(stdout,"[i] Using processname %s\n",strProcessName);
	...
	else if (strString != NULL)
	{
		...
		if(dwPID != 0 ){
			...
			EnumerateProcesses(true,true,strString,strProcessName);
		}
	} 
	else 
	{
